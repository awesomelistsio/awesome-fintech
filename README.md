# Awesome FinTech [![Awesome Lists](https://srv-cdn.himpfen.io/badges/awesome-lists/awesomelists-flat.svg)](https://github.com/awesomelistsio/awesome)

[![DOI](https://zenodo.org/badge/1116261020.svg)](https://doi.org/10.5281/zenodo.19673345)  
[![GitHub Sponsor](https://srv-cdn.himpfen.io/badges/github/github-flat.svg)](https://github.com/sponsors/brandonhimpfen) &nbsp; 
[![Buy Me a Coffee](https://srv-cdn.himpfen.io/badges/buymeacoffee/buymeacoffee-flat.svg)](https://buymeacoffee.com/brandonhimpfen) &nbsp; 
[![Ko-Fi](https://srv-cdn.himpfen.io/badges/kofi/kofi-flat.svg)](https://ko-fi.com/brandonhimpfen) &nbsp; 
[![PayPal](https://srv-cdn.himpfen.io/badges/paypal/paypal-flat.svg)](https://paypal.me/brandonhimpfen)

📌 This repository is archived with Zenodo and can be cited using the DOI above.

> A curated list of platforms, APIs, tools, frameworks, standards, and learning resources for **Financial Technology (FinTech)**, spanning payments, banking, lending, wealth management, compliance, blockchain, and financial data.

_Support ongoing maintenance and curation via [GitHub Sponsors](https://github.com/sponsors/brandonhimpfen)._

## Contents

- [Foundations & Standards](#foundations--standards)
- [Payments & Money Movement](#payments--money-movement)
- [Banking & Open Banking](#banking--open-banking)
- [Cards & Issuing](#cards--issuing)
- [Lending & Credit](#lending--credit)
- [Wealth Management & Investing](#wealth-management--investing)
- [Personal Finance & Budgeting](#personal-finance--budgeting)
- [Financial Data & Market APIs](#financial-data--market-apis)
- [Fraud, Risk & Compliance](#fraud-risk--compliance)
- [RegTech & Identity](#regtech--identity)
- [Blockchain & Crypto Infrastructure](#blockchain--crypto-infrastructure)
- [Accounting, Invoicing & Tax](#accounting-invoicing--tax)
- [Developer Tools & Infrastructure](#developer-tools--infrastructure)
- [Learning Resources](#learning-resources)
- [Related Awesome Lists](#related-awesome-lists)

## Foundations & Standards

- [ISO 20022](https://www.iso20022.org/) – Global standard for electronic data interchange between financial institutions.
- [SWIFT](https://www.swift.com/) – Secure messaging network used by banks and financial institutions worldwide.
- [PCI DSS](https://www.pcisecuritystandards.org/) – Security standards for handling cardholder data.
- [Open Banking](https://www.openbanking.org.uk/) – Framework enabling secure data sharing between banks and third parties.
- [Financial Data Exchange (FDX)](https://financialdataexchange.org/) – Common API standard for financial data sharing.

## Payments & Money Movement

- [Stripe](https://stripe.com/) – Payment processing platform for online and subscription businesses.
- [Adyen](https://www.adyen.com/) – Global payments platform for enterprises and marketplaces.
- [PayPal](https://www.paypal.com/) – Online payments and digital wallet platform.
- [Square](https://squareup.com/) – Payments and point-of-sale tools for businesses.
- [Checkout.com](https://www.checkout.com/) – Cloud-based payment processing platform.
- [Wise](https://wise.com/) – Cross-border payments with transparent exchange rates.
- [Payoneer](https://www.payoneer.com/) – Global payments platform for freelancers and marketplaces.

## Banking & Open Banking

- [Plaid](https://plaid.com/) – API platform connecting apps to users’ bank accounts.
- [Tink](https://tink.com/) – Open banking platform for payments and financial data.
- [TrueLayer](https://truelayer.com/) – Open banking APIs for payments and data access.
- [Yapily](https://www.yapily.com/) – Open banking infrastructure for financial services.
- [Solaris](https://www.solarisgroup.com/) – Banking-as-a-Service platform for embedded finance.

## Cards & Issuing

- [Marqeta](https://www.marqeta.com/) – Card issuing and payment processing platform.
- [Stripe Issuing](https://stripe.com/issuing) – APIs for creating and managing virtual and physical cards.
- [Adyen Issuing](https://www.adyen.com/card-issuing) – End-to-end card issuing solution.
- [Unit](https://www.unit.co/) – Embedded finance platform with card issuing and banking.
- [Synapse](https://synapsefi.com/) – Banking and card issuing APIs for FinTechs.

## Lending & Credit

- [Upstart](https://www.upstart.com/) – AI-driven lending and credit risk platform.
- [Affirm](https://www.affirm.com/) – Buy now, pay later (BNPL) platform.
- [Kabbage](https://www.kabbage.com/) – Small business lending and cash flow tools.
- [Lendio](https://www.lendio.com/) – Online marketplace for small business loans.
- [Zopa](https://www.zopa.com/) – Digital bank offering loans and savings products.

## Wealth Management & Investing

- [Robinhood](https://robinhood.com/) – Commission-free investing platform.
- [Wealthfront](https://www.wealthfront.com/) – Robo-advisor for automated investing.
- [Betterment](https://www.betterment.com/) – Digital wealth management platform.
- [Interactive Brokers](https://www.interactivebrokers.com/) – Trading and brokerage APIs.
- [Alpaca](https://alpaca.markets/) – API-first stock trading and brokerage platform.

## Personal Finance & Budgeting

- [Mint](https://mint.intuit.com/) – Personal finance and budgeting app.
- [YNAB](https://www.ynab.com/) – Budgeting software based on zero-based budgeting.
- [Monzo](https://monzo.com/) – Digital bank with built-in budgeting tools.
- [Revolut](https://www.revolut.com/) – Financial super app with banking, cards, and analytics.
- [Emma](https://emma-app.com/) – Personal finance app for tracking spending and subscriptions.
- [Sweepbase](https://sweepbase.net/) – Crypto debit and credit card comparison with published methodology.

## Financial Data & Market APIs

- [Bloomberg](https://www.bloomberg.com/professional/) – Financial data, analytics, and news platform.
- [Refinitiv](https://www.refinitiv.com/) – Market data and financial infrastructure provider.
- [Alpha Vantage](https://www.alphavantage.co/) – Free and paid APIs for market data.
- [Polygon.io](https://polygon.io/) – Real-time and historical financial market data APIs.
- [Quandl (Nasdaq Data Link)](https://data.nasdaq.com/) – Economic and financial datasets.

## Fraud, Risk & Compliance

- [Stripe Radar](https://stripe.com/radar) – Machine-learning-based fraud detection for payments.
- [Sift](https://sift.com/) – Fraud prevention and risk management platform.
- [Riskified](https://www.riskified.com/) – E-commerce fraud and chargeback protection.
- [Feedzai](https://feedzai.com/) – AI-powered fraud detection for financial institutions.
- [Featurespace](https://www.featurespace.com/) – Anomaly detection for financial crime.

## RegTech & Identity

- [Onfido](https://onfido.com/) – Identity verification and KYC platform.
- [Trulioo](https://www.trulioo.com/) – Global identity verification APIs.
- [Sumsub](https://sumsub.com/) – KYC, KYB, and AML compliance platform.
- [Jumio](https://www.jumio.com/) – AI-powered identity verification services.
- [ComplyAdvantage](https://complyadvantage.com/) – AML risk intelligence and screening.

## Blockchain & Crypto Infrastructure

- [Coinbase](https://www.coinbase.com/) – Cryptocurrency exchange and developer APIs.
- [Binance](https://www.binance.com/) – Crypto trading and blockchain infrastructure platform.
- [Chainalysis](https://www.chainalysis.com/) – Blockchain analytics and compliance tools.
- [Fireblocks](https://www.fireblocks.com/) – Secure digital asset custody and transfer platform.
- [Alchemy](https://www.alchemy.com/) – Blockchain developer platform and APIs.

## Accounting, Invoicing & Tax

- [QuickBooks](https://quickbooks.intuit.com/) – Accounting and bookkeeping software.
- [Xero](https://www.xero.com/) – Cloud-based accounting platform for SMBs.
- [FreshBooks](https://www.freshbooks.com/) – Invoicing and accounting for small businesses.
- [Wave](https://www.waveapps.com/) – Free accounting and invoicing tools.
- [TaxJar](https://www.taxjar.com/) – Automated sales tax calculation and filing.

## Developer Tools & Infrastructure

- [Stripe API](https://stripe.com/docs/api) – APIs for payments, billing, and financial workflows.
- [Plaid API](https://plaid.com/docs/) – Developer APIs for banking data access.
- [Open Banking APIs](https://www.openbanking.org.uk/) – Standards-based financial APIs.
- [AWS for Financial Services](https://aws.amazon.com/financial-services/) – Cloud infrastructure for FinTech workloads.
- [Google Cloud for FinTech](https://cloud.google.com/solutions/financial-services) – Cloud services for financial applications.

## Learning Resources

### Tutorials
- [FinTech Open Source Foundation (FINOS)](https://www.finos.org/) – Open-source standards and projects for finance.
- [Stripe Developer Guides](https://stripe.com/docs) – Practical guides for payment integration.
- [Plaid Learning Center](https://plaid.com/resources/) – Tutorials and use cases for open banking.

### Guides
- [Embedded Finance Explained](https://www.mckinsey.com/) – Overview of embedded financial services.
- [FinTech Regulatory Landscape](https://www.bis.org/) – Global regulatory perspectives on FinTech.
- [Open Banking Guide](https://www.openbanking.org.uk/) – Best practices for open banking adoption.

### Courses
- *Introduction to FinTech* – Overview of modern financial technologies.
- *Payments & Digital Banking* – Systems and infrastructure behind payments.
- *FinTech Product Development* – Building and scaling financial products.

## Related Awesome Lists

- [Awesome Finance](https://github.com/awesomelistsio/awesome-finance)
- [Awesome Payments](https://github.com/awesomelistsio/awesome-payments)
- [Awesome APIs](https://github.com/awesomelistsio/awesome-apis)
- [Awesome Blockchain](https://github.com/awesomelistsio/awesome-blockchain)
- [Awesome SaaS](https://github.com/awesomelistsio/awesome-saas)

## Contribute

Contributions are welcome. Please ensure your submission fully follows the requirements outlined in [`CONTRIBUTING.md`](CONTRIBUTING.md), including formatting, scope alignment, and category placement.

Pull requests that do not adhere to the contribution guidelines may be closed.

## License

[![CC0](https://mirrors.creativecommons.org/presskit/buttons/88x31/svg/by-sa.svg)](http://creativecommons.org/licenses/by-sa/4.0/)
